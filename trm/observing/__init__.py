...
SITES = {
    ...
    "GTC": {"long": "-17 53 31", "lat": "+28 45 24", "height": 2300.0, "zhole": 8.0},
    "NTT": {"long": "-70 44 00.", "lat": "-29 15 00.", "height": 2400.0, "zhole": 5.0},
    "VLT": {"long": "-70 24 9.9", "lat": "-24 37 30.3", "height": 2635.0, "zhole": 4.0},
    "Keck": {
        "long": "-155 28 30.04",
        "lat": "+19 49 34.9",
        "height": 4145.0,
    ...
    "TNT": {"long": "+98 28 00", "lat": "+18 34 00", "height": 2457.0, "zhole": 2.0},
    "A2.3": {"long": "+22 11 46", "lat": "+37 59 04", "height": 2340.0, "zhole": 2.0},
    "SAAO": {"long": "+20 48 42", "lat": "-32 23 14", "height": 1798.0, "zhole": -1.0},
    "Magellan": {
        "long": "-70 42 05.9",
        "lat": "-29 00 35.8",
        "height": 2275.0,
...
class Ephemeris(object):
    ...
    def phase(self, time):
        ...
        if self.poly == "quadratic":
            pold = pnew - 1
            while np.abs(pnew - pold) > 1.0e-8:
                pold = pnew
                pnew = (time - self.coeff[0] - self.coeff[1] * pold**2) / self.coeff[1]
        return pnew
...
class Switch(object):
    ...
    def __init__(self, line):
        ...
        utc = float(utv[0])
        if len(utv) > 1:
            utc += float(utv[1]) / 60.0
        if len(utv) > 2:
            utc += float(utv[2]) / 3600.0
        self.utc = utc
        self.delta = float(delta) / 60.0
                ...
                if line.startswith("#") or line.isspace():
                    if count:
                        if name in peinfo:
                            ...
                            print(
                                name,
                                "not found in position/ephemeris file and will be skipped.",
                            )
                        count = 0
                else:
                    count += 1
...
def tnt_alert(alt, az):
    ...
    AZ = np.radians(np.array([25.0, 35.5, 54.5]))
    calt, salt = np.cos(ALT), np.sin(ALT)
    caz, saz = np.cos(AZ), np.sin(AZ)
    v = CartesianRepresentation(x=saz * calt, y=caz * calt, z=salt)
    ...
    v = CartesianRepresentation(
        x=m.sin(raz) * m.cos(ralt), y=m.cos(raz) * m.cos(ralt), z=m.sin(ralt)
    )

    return a1.dot(v) > 0 and a2.dot(v) > 0
...
def sun_at_alt(time1, time2, site, alt, tol=0.05):
    ...
    if (sunaltaz1.alt.value < alt and sunaltaz2.alt.value < alt) or (
        ...
        raise Exception(
            "sun_at_alt: times do no bracket the Sun crossing altitude = " + str(alt)
        )
    ...
    while abs(sunaltaz2.alt.value - sunaltaz1.alt.value) > tol:
        tmid = time.Time((time1.mjd + time2.mjd) / 2.0, format="mjd")
        altazframe = AltAz(obstime=tmid, location=site)
        sun = get_sun(tmid)
        sunaltaz = sun.transform_to(altazframe)

        if (sunaltaz.alt.value < alt and rising) or (
            sunaltaz.alt.value > alt and not rising
        ):
            sunaltaz1 = sunaltaz
            time1 = tmid
        else:
